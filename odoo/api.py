...
class Cache(object):
    ...
    def __init__(self):
        # {field: {record_id: thing}}, where 'thing' is either the field's
        # value, or a dictionary {key: value} for context-dependent fields
        self._data = defaultdict(dict)

    def _get_context_key(self, env, field):
        return tuple(env.context.get(key) for key in field.depends_context)
