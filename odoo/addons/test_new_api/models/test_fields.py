...
class DummyCompute(models.Model):
    _name = 'test_new_api.dummy.compute'
    _description = 'basic model with two computed fields (with & without defaults)'
    _inherit = 'test_new_api.dummy'
    _pre_compute=True

    default = fields.Char(compute="_compute_default", store=True)
    basic = fields.Char(compute="_compute_basic", store=True)
...
class DummyComputeEditable(models.Model):
    _name = 'test_new_api.dummy.compute.editable'
    _description = 'basic model with two editable computed fields (with & without defaults)'
    _inherit = 'test_new_api.dummy.compute'

    default = fields.Char(readonly=False)
    basic = fields.Char(readonly=False)

class DummyComputeInverse(models.Model):
    _name = 'test_new_api.dummy.compute.inverse'
    _description = 'basic model with two editable (bc inverse) computed fields (with & without defaults)'
    _inherit = 'test_new_api.dummy.compute'

    default = fields.Char(inverse="_inverse_default")
    basic = fields.Char(inverse="_inverse_basic")

    default_inversed = fields.Boolean()
    basic_inversed = fields.Boolean()

    def _inverse_default(self):
        self.default_inversed = True

    def _inverse_basic(self):
        self.basic_inversed = True
