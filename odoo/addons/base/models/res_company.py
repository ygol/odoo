...
class Company(models.Model):
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        # optimization of searches like [('parent_id', 'in', ids)] generated by
        # access rules with a comain like [('company_id', 'child_of', [id])]
        if len(args) == 1 and args[0][0] == 'parent_id' and args[0][1] == 'in' and not count:
            children = self._children_ids()
            return [cid for pid in args[0][2] for cid in children[pid]]
        return super()._search(args, offset, limit, order, count, access_rights_uid)

    @tools.ormcache('self.env.uid')
    def _children_ids(self):
        """ Return a mapping from company ids to their corresponding children ids. """
        children = defaultdict(list)
        for company in self.with_context(prefetch_fields=False).search([]):
            children[company.parent_id.id].append(company.id)
        return children
