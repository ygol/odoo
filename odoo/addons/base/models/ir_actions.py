...
class IrActionsTodo(models.Model):
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100):
        args = args or []
        if name:
            action_ids = self._search(expression.AND([[('action_id', operator, name)], args]), limit=limit)
            return models.lazy_name_get(self.browse(action_ids))
        return super(IrActionsTodo, self)._name_search(name, args=args, operator=operator, limit=limit)
