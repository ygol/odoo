...
class MailComposer(models.TransientModel):
    ...
    mail_server_id = fields.Many2one(
        ...
        compute='_compute_mail_server_id', compute_sudo=False, readonly=False, store=True)

    @api.depends('template_id', 'composition_mode', 'parent_id', 'record_name')
    def _compute_subject(self):
        if self.template_id.subject:
            self._get_compute_value_from_template('subject')
        elif not self.subject:
            subject = self.parent_id.subject if self.parent_id else False
            if not subject:
                subject = self.record_name if self.record_name else False
            if subject:
                re_prefix = _('Re:')
    ...
    def _compute_body(self):
        if self.template_id.body_html:
            self._get_compute_value_from_template('body_html', 'body')
