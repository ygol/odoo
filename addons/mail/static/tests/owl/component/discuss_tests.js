...
QUnit.test('basic rendering of message', async function (assert) {
    ...
    assert.strictEqual(
        message
            ...
            .dataset
            .src,
        "/web/image/res.partner/11/image_64",
...
QUnit.test('composer state: text save and restore', async function (assert) {
    ...
    const editableSelector = `
        ...
        > .note-editing-area
        > .note-editable`;
    const generalEditable = document.querySelector(editableSelector);
    // Write text in composer for #general
    generalEditable.innerHTML = "<p>XDU for the win</p>";
    const kevt = new window.KeyboardEvent('input');
    generalEditable.dispatchEvent(kevt);
    await testUtils.nextTick();
    // Switch to #special
    await testUtils.dom.click(channels[1]);
    // Write text in composer for #special
    const specialEditable = document.querySelector(editableSelector);
    specialEditable.innerHTML = `They see me rollin'`;
    const kevt2 = new window.KeyboardEvent('input');
    specialEditable.dispatchEvent(kevt2);
    await testUtils.nextTick();
    // Switch back to #general
    await testUtils.dom.click(channels[0]);
    assert.strictEqual(document.querySelector(editableSelector).innerHTML,
        "<p>XDU for the win</p>",
...
QUnit.test('composer state: attachments save and restore', async function (assert) {
    ...
    assert.strictEqual(
        document
            ...
            .attachmentLocalId,
        'ir.attachment_2',
        "should have ir.attachment_2 attachment as 1st attachment");
    assert.strictEqual(
        document
            .querySelectorAll(`
                .o_Composer_attachmentList
            ...
            .attachmentLocalId,
        'ir.attachment_3',
        "should have ir.attachment_3 attachment as 2nd attachment");
    assert.strictEqual(
        document
            .querySelectorAll(`
                .o_Composer_attachmentList
            ...
            .attachmentLocalId,
        'ir.attachment_4',
        "should have ir.attachment_4 attachment as 3rd attachment");
});
