odoo.define('pos_coupon.tour.pos_coupon1', function (require) {
    ...
    // applied programs:
    //   - on cheapest product
    ProductScreen.exec.order('Whiteboard Pen', '5');
    ...
    PosCoupon.exec.finalizeOrder('Cash', '13.12');

    // remove the reward from auto promo program
    // no applied programs
    ProductScreen.exec.order('Whiteboard Pen', '6');
    ...
    PosCoupon.exec.finalizeOrder('Cash', '19.2');

    // order with coupon code from coupon program
    // applied programs:
    //   - coupon program
    ProductScreen.exec.order('Desk Organizer', '9');
    ...
    PosCoupon.exec.finalizeOrder('Cash', '30.6');

    // Use coupon but eventually remove the reward
    // applied programs:
    //   - on cheapest product
    ProductScreen.exec.order('Letter Tray', '4');
    ProductScreen.exec.order('Desk Organizer', '9');
    ...
    PosCoupon.check.orderTotalIs('62.27');
    PosCoupon.exec.finalizeOrder('Cash', '62.27');

    // specific product discount
    // applied programs:
    //   - on cheapest product
    //   - on specific products
    ProductScreen.exec.order('Magnetic Board', '10') // 1.98
    ProductScreen.exec.order('Desk Organizer', '3') // 5.1
    ...
    PosCoupon.exec.finalizeOrder('Cash', '36.89')
