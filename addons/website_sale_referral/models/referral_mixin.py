    ...
    def get_referral_statuses(self, utm_source_id, referred_email=None):
        ...
        for o in objects:
            ...
            if(o.referred_email not in result or self.STATES_PRIORITY[state] > self.STATES_PRIORITY[result[o.referred_email]['state']]):
                result[o.referred_email] = {'state': state, 'name': o.referred_name or '', 'company': o.referred_company or ''}
    ...
    def check_referral_progress(self, old_state, new_state):
        ...
        if new_state != old_state and not len(others_to_reward):
            ...
            if new_state == 'done':
                ...
                self._send_mail('referral_cancelled_email_template', 'Referral lost...')
            else:
                self._send_mail('referral_state_changed_email_template', 'Referral moved forward !', {'referred_name': self.referred_name, 'tracking_link': referral_tracking.get_tracking_link()})
