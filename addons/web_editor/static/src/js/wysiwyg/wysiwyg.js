...
var Wysiwyg = Widget.extend({
    ...
    /*
     * return true if the editor display the popover
     *
     * @param {DOM} node
     * @returns {Boolean}
     */
    _displayPopover: function (node) {
        return true;
    },
    /**
     * @private
    ...
    _editorOptions: function () {
        ...
        options.displayPopover = this._displayPopover.bind(this);
    ...
    o_editable: function (node, i, m) {
        while (node) {
            if (node.className && _.isString(node.className)) {
                if (node.className.indexOf('o_not_editable') !== -1 || node.className.indexOf('media_iframe_video') !== -1 || (node.attributes.contenteditable && node.attributes.contenteditable.value !== 'true')) {
                    return false;
                }
