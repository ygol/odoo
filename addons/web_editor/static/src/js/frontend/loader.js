 ...
 */
async function createWysiwyg(parent, options) {
    const assetLib = options.lib === 'jabberwock' ?
        'web_editor.compiled_assets_jabberwock_wysiwyg' :
        'web_editor.compiled_assets_wysiwyg';
    if (!loadedLib.has(assetLib)) {
        await ajax.loadLibs({ assetLibs: [ assetLib, 'website.compiled_assets_wysiwyg' ] });
