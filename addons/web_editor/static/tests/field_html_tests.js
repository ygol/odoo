...
var keyboardTestsTab = [
    { name: "TAB at start of list element (must leave field)",
        ...
        test: {
            check: function ($editable, assert) {
                assert.strictEqual($editable.html(), "<ul><li>dom to edit</li></ul>", "The DOM don't change");
    ...
    { name: "TAB within table element (must leave field)",
        ...
        test: {
            check: function ($editable, assert) {
                assert.strictEqual($editable.html(), "<table><tbody><tr><td>dom to edit</td><td>node</td></tr></tbody></table>", "The DOM don't change");
                $editable.mousedown().focus().mouseup();
            },
        }
    },
...
QUnit.test('Tab must switch to an other field', function (assert) {
    var done = assert.async();
    testUtils.createAsyncView({
        View: FormView,
        model: 'note.note',
        ...
        testUtils.intercept(form.renderer, "wysiwyg_focus", function (ev) {
            assert.step("wysiwyg_focus");
        });
        testUtils.intercept(form.renderer, "navigation_move", function (ev) {
...
QUnit.test('rendering with snippet panel', function (assert) {
    ...
    testUtils.createAsyncView({
        ...
        testUtils.intercept(form, "snippets_loaded", function () {
...
QUnit.test('drag&drop snippet options', function (assert) {
    ...
    testUtils.createAsyncView({
        ...
        testUtils.intercept(form, "snippet_focused", function (e) {
        ...
        defSnippets.then(function () {
            ...
            defActivateSnippet.then(function (snippet) {
                ...
                defActivateSnippet.then(function (snippet) {
                    assert.strictEqual($editable.html().replace(/\s+/g, ' '),
                        '<div class=\"s_hr pt32 pb32\"> <hr class=\"s_hr_1px s_hr_solid w-100 mx-auto\"> </div><div class=\"s_hr pt32 pb32\"> <hr class=\"s_hr_1px s_hr_solid w-100 mx-auto\"> </div><p>toto toto toto</p><p>tata</p>',
                        "should drop the snippet");

                    assert.strictEqual(snippet.$target.index(), 1,
                        'should show the snippet editor for the second dropped block');

                    assert.strictEqual($('.oe_active .oe_overlay_options .btn:visible', doc).length, 3,
