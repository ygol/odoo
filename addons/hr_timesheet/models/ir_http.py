...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        if self.env.user.has_group('base.group_user'):
            ...
            # self.env['uom.uom'].browse(int(self.env['ir.config_parameter'].sudo().get_param('hr_timesheet.timesheet_encode_uom_id')))
            project_time_mode = self.env['uom.uom']._get_uom_by_config_parameter('hr_timesheet.project_time_mode_id')
            # self.env['uom.uom'].browse(int(self.env['ir.config_parameter'].sudo().get_param('hr_timesheet.project_time_mode_id')))
