...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_processing_values(self, processing_values):
        ...
        converted_amount = self.acquirer_id._adyen_convert_to_minor_units(
            self.amount, self.currency_id.name
        )
        return {
            'converted_amount': converted_amount,
            'access_token': payment_toolbox.generate_access_token(
    ...
    def _send_payment_request(self, _operation='online'):
        ...
        for tx in self:
            ...
            data = {
                ...
                'amount': {
                    'value': acquirer._adyen_convert_to_minor_units(tx.amount, tx.currency_id.name),
                    'currency': tx.currency_id.name,  # ISO 4217
                },
