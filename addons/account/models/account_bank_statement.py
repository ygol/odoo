...
class AccountBankStatement(models.Model):
    ...
    def write(self, values):
        ...
        if values.get('date') or values.get('journal'):
            ...
            to_recompute = self.search([('previous_statement_id', 'in', self.ids), ('id', 'not in', self.ids), ('journal_id', '=', self.journal_id.id)])
            if to_recompute:
                self.env.add_to_compute(self._fields['previous_statement_id'], to_recompute)
                to_recompute.modified(['previous_statement_id'])
            next_statements_to_recompute = self.search([('previous_statement_id', 'in', [st.previous_statement_id.id for st in self]), ('id', 'not in', self.ids), ('journal_id', '=', self.journal_id.id)])
    ...
    def unlink(self):
        for statement in self:
            ...
            next_statement = self.search([('previous_statement_id', '=', statement.id), ('journal_id', '=', self.journal_id.id)])
            if next_statement:
