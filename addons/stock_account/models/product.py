    ...
    def _prepare_out_svl_vals(self, quantity, company):
        ...
        if self.cost_method in ('average', 'fifo'):
            ...
            if self.cost_method == 'average':
                futur_quantity_svl = self.quantity_svl + quantity
                futur_value_svl = self.value_svl + (self.standard_price * quantity)
                rounding_error = (self.standard_price * futur_quantity_svl) - futur_value_svl
                if not float_is_zero(rounding_error, precision_rounding=self.env.company.currency_id.rounding):
