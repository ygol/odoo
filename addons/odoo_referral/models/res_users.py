...
class Users(models.Model):
    ...
    def get_referral_updates_count_for_current_user(self):
        ...
        if(not user_id.referral_updates_last_fetch_time or user_id.referral_updates_last_fetch_time > fields.Datetime.now() + timedelta(days=1)):
            payload = {
                'jsonrpc': '2.0',
                'method': 'call',
                'params': {},
            ...
            result = requests.post(DEST_SERVER_URL + '/referral/notifications/' + user_id._get_referral_token(), json=payload, headers={'content-type': 'application/json'}).json()
            user_id.referral_updates_last_fetch_time = fields.Datetime.now()
            if('result' in result and 'updates_count' in result['result']):
                user_id.referral_updates_count = result['result']['updates_count']
            else:
                user_id.referral_updates_count = 0
        return user_id.referral_updates_count
