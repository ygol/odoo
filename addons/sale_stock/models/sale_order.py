    ...
    def _compute_json_forecast(self):
        ...
        for so_line in self:
            ...
            if any(move.state == 'done' for move in so_line.move_ids) or not so_line.move_ids.picking_id:
                ...
                move_datetimes_expected = [fields.Datetime.to_datetime(m) for m in move_datetimes_expected if m]
                datetime_expected = max(move_datetimes_expected, default=False)
                so_deadline = self.order_id.commitment_date or so_line.order_id.expected_date or False
