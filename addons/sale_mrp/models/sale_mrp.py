...
class SaleOrderLine(models.Model):
    ...
    def _get_forecastable_lines(self):
        ...
        (storable product or kit product) """
        manufactured_lines = self.filtered(
            lambda l: l.bom_count > 0 and
            float_compare(l.qty_to_deliver, 0, precision_rounding=l.product_id.uom_id.rounding) > 0
        )
        res = super(SaleOrderLine, self)._get_forecastable_lines()
        return res | manufactured_lines
